...
export function generateEmptyPlaceholderHistory() {
  const history = [];
  const now = new Date();
  for (let i = 13; i >= 0; i--) {
...
export function StreakTracker({
  ...
  const effectiveHistory = streakHistory && streakHistory.length > 0 
    ? streakHistory 
    : generateEmptyPlaceholderHistory();
  ...
  return (
    <Card className="p-5 bg-surface border-border shadow-sm space-y-4">
      ...
      <div className="pt-2 border-t border-border space-y-2">
        ...
        {isNewLearner ? (
          <div className="space-y-2">
            <div className="grid grid-cols-7 sm:grid-cols-14 gap-1.5 opacity-60">
              {effectiveHistory.map((day) => {
                ...
                return (
                  <div
                    key={day.date}
                    className="p-2 rounded-md border border-dashed border-border text-center flex flex-col items-center justify-between min-h-[54px] bg-canvas/30 text-slate/40"
                    title={`${day.date}: No activity recorded yet`}
                  >
                    <span className="text-[9px] text-slate font-mono uppercase">{monthStr} {dayNum}</span>
                    <span className="font-mono text-xs font-bold mt-0.5">—</span>
                    <span className="w-1.5 h-1.5 rounded-full mt-1 bg-border" />
                  </div>
                );
              })}
            </div>
            <p className="text-[11px] text-slate text-center italic">
              Empty placeholder days: Activity tracking begins when you complete your first drill.
            </p>
          </div>
        ) : (
